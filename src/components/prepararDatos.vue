<template>
  ...
  <v-content>
    ...
    <div v-else>
      <v-alert :value="true" icon="info" color="primary" outline dismissible class="mt-5">
        Recuerda...</strong><br>El formato que el formato que debes adjuntar debe ser ".csv UTF8 (delimitado por comas)" de excel y
        debe cumplir con los campos obligatorios: fecha, hora, solicitud, dependencia, respuesta y observaciones. Por ultimo
        no olvide que todos los campos anteriores deben estar llenos, si en alguno de ellos no desea poner información entonces
        rellene con un "-", si no lo hace, el formato sera lanzado como inválido.
      </v-alert>
    </div>
    ...
    <div class="text-xs-justify pt-4">
      <v-btn color="black" left absolute outline to="/steps">Atrás</v-btn>
      <v-btn color="black" right absolute outline @click="next('/UsingIA/DefinirModelo')">Siguiente</v-btn>
    </div>
  </v-content>
  </v-container>
</template>
