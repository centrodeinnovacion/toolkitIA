...
const mutations = {
  ...
  changeElements: (state, data) => {
    state.model.contexto.modo = state.application.mode
  ...
  retrieveCM: (state, data) => {
    state.application.createModel = false
    state.application.mode = 'DIY'
    state.application.currentCognitiveModel = data.key
    state.model.contexto = data.cm
  },
  deleteWord: (state, index) => {
    state.model.contexto.palabrasClave.splice(index, 1)
  },
  deleteCategory: (state, data) => {
    state.model.relaciones[data.c].temasDeInteres.splice(data.i, 1)
  }
}
...
const actions = {
  ...
  updatingCognitiveModel (context) {
    db.collection('cognitiveModels').doc(context.state.application.currentCognitiveModel).update(context.state.model.contexto).then(updt => EventBus.$emit('loading', {loading: false}))
    router.push('/UsingIA/palabraClave')
  },
  relationedWords (context, link) {
    ...
    router.push('/UsingIA/questionsList')
    EventBus.$emit('loading', {loading: false})
  },
  ...
  loadCognitiveModel (context, id) {
    if (context.state.application.authenticated === true) {
      db.collection('cognitiveModels').doc(id).get().then(CognitiveModel => {
        ...
        try {
          context.commit('retrieveCM', { cm: CognitiveModel.data(), key: CognitiveModel.id })
        } catch (error) {
          console.log(error)
        } finally {
          router.push('UsingIA/DefinirModelo')
          EventBus.$emit('loading', {loading: false})
        }
      }).catch(error => EventBus.$emit('errorMessage', { text: `Error al registrar el Modelo cognitivo: ${error}`, title: 'Error de registro deL Modelo', boolean: true }))
    }
